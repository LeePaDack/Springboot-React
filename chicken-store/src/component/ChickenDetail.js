...
const ChickenDetail = () => {
    ...
    // 수정하기 버튼을 눌렀는지 안눌렀는지 체키 boolean
    ...
    const handle수정내용저장 = () => {
        ...
        .then(response => {
            setChicken(response.data) // 기존에 DB 에 저장된 내용 가져오기
            setEditData(false);
            setIsEditing(false);
        })
        .catch(error => {
            console.error("수정하는데 문제 발생", error);
        })
    }
    ...
    return (
        <div className="chicken-detail-container">
            {/* 수정하기버튼을누르면 ? 수정하는 기능 가져오기 : 안누르면 작성된 내용 보여주기 */}
            {isEditing ? ( 
            <div>
                ...
                <button onClick={handle수정내용저장}>수정완료</button>
                <button onClick={handleBack}>돌아가기</button>
            </div>) : (<div>
                <h1>{chicken.chickenName}</h1>
                <p>{chicken.description}</p>
                <p>₩ {chicken.price.toLocaleString()}</p>
                <button onClick={handle수정하기}>수정하기</button>
                <button onClick={handleDelete}>삭제하기</button>
                <button onClick={handleGoMain}>돌아가기</button>
            </div>)
