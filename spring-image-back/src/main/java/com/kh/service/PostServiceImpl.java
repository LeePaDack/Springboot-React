...
public class PostServiceImpl implements PostService{
	...
	@Value("${file.upload-dir}") // application.properties 에서 설정 이름 가져와
	private String uploadDir;
	// private String uploadDir = "C:/Users/user1/Desktop/saveImage";
	// C:/Users/user1/Desktop/saveImage 경로 넣어서 사용
	...
	public void uploadImages(MultipartFile[] files, String title, String content) {
		...
		if(!uploadDirFile.exists()) {
			...
			if (!uploadDirFile.mkdirs()) {
				...
				// mkdir = 폴더 1 개 mkdirs = 하위 폴더 모두 생성
			}
		} 
		// UUID 이미지 이름 저장 중복없이 저장할 수 있도록 설정
		List<String> fileNames = null; // 파일명이 1 개일 수 있고, 여러 이름이 들어올 수 있기 때문에
		// fileNames = 파일이름들 리스트로 글자목록이 작성
		
		try {
			// MultipartFile[] files  = array 배열로 파일들 담기
			// List.of(files)   files = 파일들 배열을 리스트로 변환
			// .stream() = 리스트나 배열과 같은 데이터를 하나씩 처리하는 기능
			// .collect(Collectors.toList());   stream 으로 가져온 이미지 데이터를 리스트로 정렬
			
			// 이미지들 이미지를 한개씩 담아오기 map -> 이미지를 하나씩 가져올 때 stream 을 이용해서 가져오고
			// stream 을 이용해서 가져온 이미지를 collect 를 이용해서 리스트로 모음
			// 한번 더 리스트로 목록 변환
			fileNames = List.of(files).stream().map(file -> {
				// 파일을 폴더에 저장 폴더에 파일을 저장할 때 이미지에 랜덤으로 이름을 부여해서 저장
				// UUID 를 이용해서 저장 = 파일 이름이 겹치지 않도록 랜덤으로 이름 생성
				String fileName1 = UUID.randomUUID().toString();
				
				// 랜덤으로 작성한 이름 뒤에 원본 이름을 붙이고 싶을 때
				// file.getOriginalFileName() 을 + 로 붙여주면 됨
				String fileName2 = UUID.randomUUID().toString() + file.getOriginalFilename();
				
				// 랜덤으로 작성한 이름 _ 원본이름 _ 으로 랜덤이름과 원본 이름 구분짓고 싶을 때
		...
		// 이미지 이름 설정과 이미지 경로 생성한 것을 바탕으로 DB 에 넣어주기
